...
module.exports = React.createClass({
    ...
    getUserInfo: function() {
        let ppToken = Demo.ppToken || Demo.conn.ppToken;
        fetch(
            PP_USER + Demo.userInfo.userId,
            {
                method:"GET",
                headers:{
                    "Accept":"*/*",
            ...
            res.text().then( r => {
                Demo.userInfo = JSON.parse(r);
                Demo.avatars[Demo.user] = Demo.userInfo.avatar || null;
            })
        })
    },
        ...
        if (WebIM.config.isWindowSDK) {
            ...
            conn.getRoster({
                success: function (roster) {
                    ...
                    for (var i in roster) {
                        var ros = roster[i];
                        ros.name = ros.toAccount;
                        ros.jid  = `1166171023115752#icmp_${ros.name}@easemob.com`
                        
                        friends.push(ros);
                        Demo.roster[ros.name] = 1;
                        Demo.remarks[ros.name] = ros;
                        if(ros.avatar) Demo.avatars[ros.name] = ros.avatar;
                    }
        ...
        if (Demo.selected) {
            ...
            switch (cate) {
                case 'friends':
                    ...
                    var remark = Demo.remarks[id].remark;
