...
var _login = function (options, conn) {
    ...
    conn.context.ppToken = conn.ppToken;
 ...
 * @param {String} pwd
 */
connection.prototype.getPpToken = async function(apiUrl,userId, pwd, suc){
    
    await fetch(
        apiUrl,
        {
            method:"POST",
            body:`{"username":${userId},"pwd":${pwd}}`,
            headers:{
                "Accept":"*/*",
...
connection.prototype.login = function (options) {
    ...
    if (options.accessToken) {
        ...
        var suc = function (data, xhr) {
            if(data.ppToken){
                conn.ppToken = data.ppToken;
                Demo.userInfo = data.userInfo;
                return;
            }
            conn.context.status = _code.STATUS_DOLOGIN_IM;
            conn.context.restTokenData = data;
            const ppInfo = { ppToken: conn.ppToken, userInfo: Demo.userInfo }
            if (options.success)
                options.success(data, ppInfo);
        ...
        conn.getPpToken(PP_AUTH, userId, pwd, suc);
        _utils.ajax(options2);
    }
};
...
connection.prototype.getRoster = function (options) {
    ...
    fetch(
        ...
        res.text().then( r => suc(JSON.parse(r)) )
    })
    ...
    if (this.isOpened()) {
        this.context.stropheConn.sendIQ(dom.tree(), getConcats, failFn);
